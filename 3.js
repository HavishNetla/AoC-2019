...
for (let i = 0; i < input1.length; i += 1) {
  const direction = input1[i].substring(0, 1)
  const dist = parseInt(input1[i].substring(1), 10)
  if (direction === 'U') {
    velX = 0
    velY = 1
  } else if (direction === 'L') {
  ...
  for (let j = 0; j < dist; j += 1) {
    pos[0] += velX
    pos[1] += velY
    inputArr1.push([pos[0], pos[1]])
  }
}

pos = [0, 0]
for (let i = 0; i < input2.length; i += 1) {
  const direction = input2[i].substring(0, 1)
  const dist = parseInt(input2[i].substring(1), 10)
  ...
  for (let j = 0; j < dist; j += 1) {
    pos[0] += velX
    pos[1] += velY
    inputArr2.push([pos[0], pos[1]])
  }
}

// For (let i = 0, len = inputArr1.length; i < len; i += 1) {
//   for (let j = 0, len1 = inputArr1.length; j < len1; j += 1) {
//     if (inputArr1[i].toString() === inputArr2[i].toString()) {
//       intersections.push(inputArr1[i])
//     }
//   }
// }
...
const intersections = inputArr1.filter(val => isItemInArray(inputArr2, val))

const manhat = arr => Math.abs(arr[0]) + Math.abs(arr[1])

let dist = Infinity
for (let i = 0; i < intersections.length; i += 1) {
  if (dist > manhat(intersections[i])) {
    dist = manhat(intersections[i])
  }
}
...
for (let j = 0; j < intersections.length; j += 1) {
  for (let i = 0; i < inputArr1.length; i += 1) {
    if (inputArr1[i].toString() === intersections[j].toString()) {
      ar.push(i + 1)
    }
  }
}
...
for (let j = 0; j < intersections.length; j += 1) {
  for (let i = 0; i < inputArr2.length; i += 1) {
    if (inputArr2[i].toString() === intersections[j].toString()) {
      ar2.push(i + 1)
    }
  }
}
...
console.log(Math.min.apply(null, ar3))
